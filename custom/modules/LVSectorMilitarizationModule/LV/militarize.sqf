/*
		...
		default: 	nul = [this] execVM "custom\modules\LVSectorMilitarizationModule\LV\militarize.sqf";

		custom:		nul = [target, side, radius, spawn men, spawn vehicles, still, men ratio, vehicle ratio,
							skills, group, custom init, ID] execVM "custom\modules\LVSectorMilitarizationModule\LV\militarize.sqf";
...
EXAMPLE: nul = [this,2,50,[true,true],[true,false,true],false,[10,0],0.1,[0.2,0.2,0.2,0.85,0.9,0.75,0.1,0.6,1,1],nil,nil,13] execVM "custom\modules\LVSectorMilitarizationModule\LV\militarize.sqf";
*/
	...
	for "_i" from 1 to _menAmount do{
		...
		if(!_still)then{
			if(_unitType in _menArray)then{
				nul = [_unit,_cPos,_radius,_doorHandling] execVM "custom\modules\LVSectorMilitarizationModule\LV\patrol-vD.sqf";
			}else{
				nul = [_unit,_pos] execVM 'custom\modules\LVSectorMilitarizationModule\LV\patrol-vH.sqf';
			};
		};
			...
			if(_side > 0)then{
				if(surfaceIsWater _pos)then{
					if(_vehicles select 1)then{
						...
						if(!_still)then{nul = [vehicle _driver,_pos] execVM 'custom\modules\LVSectorMilitarizationModule\LV\patrol-vH.sqf';};
						_validPos = true;
					};
				}else{
					if((_vehicles select 0)&&(_vehicles select 2))then{
						...
						if(_whichOne < 3)then{
							...
							if(!_still)then{nul = [_driver,_pos,[200,200]] execVM 'custom\modules\LVSectorMilitarizationModule\LV\patrol-vE.sqf';};
							vehicle _driver flyInHeight 10;
							_validPos = true;
						}else{
							_driver = [_pos, (_side - 1)] call LV_fullLandVehicle;
							if(!_still)then{nul = [vehicle _driver,_pos] execVM 'custom\modules\LVSectorMilitarizationModule\LV\patrol-vE.sqf';};
							_validPos = true;
						};
					}else{
				...
				if(!surfaceIsWater _pos)then{
					...
					if(!_still)then{nul = [_driver,_pos] execVM 'custom\modules\LVSectorMilitarizationModule\LV\patrol-vE.sqf';};
					_validPos = true;
				};
			};
