...
_sectorPosessionBLUFOR = 0;
_sectorPosessionOPFOR = 0;
{
	...
	if(_activationBLUFORcount > 0 || _sectorBLUFORcount > 0 || _sectorOPFORcount > 0) then {
		...
		if(_x getVariable "side" == east) then {
			_sectorPosessionOPFOR = _sectorPosessionOPFOR + 1;
		} else {
			_sectorPosessionBLUFOR = _sectorPosessionBLUFOR + 1;
		};
	};
} forEach CTI_SECTOR_OBJECTS;

if([_sectorPosessionBLUFOR, _sectorPosessionOPFOR, count CTI_SECTOR_OBJECTS] call gameEndingRule) then {
