revive_ui = compile preprocessFileLineNumbers "custom\modules\FarReviveModule\FarRevive\FAR_revive_ui.sqf";
...
while { true } do {
	waitUntil { ( player getVariable "FAR_deathScene") == 1 };
	...
	waitUntil { ((player getVariable "FAR_deathScene") == 0) || ((player getVariable "FAR_isUnconscious") == 1) };
	closeDialog 0;
	waitUntil {!dialog};
	if ((player getVariable "FAR_deathScene") != 0) then {
		[] spawn revive_ui;
		waitUntil {dialog};
	};
